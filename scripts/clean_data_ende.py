"""Clean the en-de parallel training corpus.

Same thresholds as clean_data_enfr.py — German is Latin-script like French,
so the word-tokenize + latin-ratio filter works unchanged. Only the default
paths change (tgt = de instead of fr).

WMT14 de-en raw is ~4.5M pairs. Expect a higher keep rate than en-fr's 30M/40M
since the de-en corpus is smaller and cleaner (less Common Crawl noise).
"""
...
def main():
    ...
    parser.add_argument("--tgt", default="data/train.de")
    parser.add_argument("--out-src", default="data/train.clean.en")
    parser.add_argument("--out-tgt", default="data/train.clean.de")
